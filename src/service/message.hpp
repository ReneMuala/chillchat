...
namespace service {
template <typename S> class message {
  S &storage;
...
public:
  ...
  int insert(model::message &message) {
    ...
    if (message.channel_id and message.user_id and
        not storage
                .template get_all<model::user_channel>(where(
                    c(&model::user_channel::user_id) == message.user_id and
                    c(&model::user_channel::channel_id) == message.channel_id))
                .empty()) {
      message.created_at =
